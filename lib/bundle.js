  ...
  _createClass(DrawingArea, [{
    ...
    value: function handleMouseMove(event) {
      ...
      for (var i = 1; i < this.axes + 1; i++) {
        var _MathUtil$rotate = MathUtil.rotate(this.midPt.x, this.midPt.y, i, this.axes),
            x1 = _MathUtil$rotate.x,
            y1 = _MathUtil$rotate.y;

        var _MathUtil$rotate2 = MathUtil.rotate(this.oldPt.x, this.oldPt.y, i, this.axes),
            x2 = _MathUtil$rotate2.x,
            y2 = _MathUtil$rotate2.y;

        var _MathUtil$rotate3 = MathUtil.rotate(this.oldMidPt.x, this.oldMidPt.y, i, this.axes),
            x3 = _MathUtil$rotate3.x,
            y3 = _MathUtil$rotate3.y;
    ...
    value: function toggleColor() {
      ...
      return function (event) {
        _this3.rainbow = false;
        document.getElementById('rainbow').checked = false;
        _this3.color = event.target.style.backgroundColor;
        document.getElementById('current-color').style.backgroundColor = _this3.color;
      };
    }
  }, {
    key: 'enterFullscreen',
    ...
    value: function toggleFullScreen() {
      if (this.fullscreen) {
        ...
        this.canvas.width = window.innerHeight - 150;
        this.init();
      }
